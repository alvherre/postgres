...
l1:
	...
	else if (result == HeapTupleBeingUpdated && wait)
	{
		 ...
		 * on the xwait; that would lead to us sleeping on our own
		 * transaction, which is unlikely to end up well.
		 *
		...
		else
		{
			...
			if (infomask & HEAP_XMAX_IS_MULTI)
			{
				 ...
				 * xact or other subxacts of this backend.	 It is legal for
				 * us to delete the tuple in either case, however (the latter
...
l3:
	...
	else if (result == HeapTupleBeingUpdated)
	{
		...
		if (mode == LockTupleKeyShare)
		{
			 ...
			 * the key going away.	This locking of future versions could
			 * block momentarily, if a concurrent transaction is deleting a
			 * key; or it could return a value to the effect that the
			 * transaction deleting the key has already committed.	So we do
			 * this before re-locking the buffer; otherwise this would be
			 * prone to deadlocks.
			 *
		...
		else if (mode == LockTupleNoKeyExclusive)
		{
			...
			if (infomask & HEAP_XMAX_IS_MULTI)
			{
				...
				else
				{
					...
					{
						/*
						 * If the lock holder is ourselves, no need to wait.
						 * Note that the lock we hold cannot possibly be as
						 * strong or stronger than what we're requesting, or
						 * we wouldn't be here, per optimization above;
						 * but if we already hold LockTupleShare then we can
						 * continue.
						 */
						if (TransactionIdIsCurrentTransactionId(members[i].xid))
						{
							Assert(members[i].xid <
								   MultiXactStatusForNoKeyUpdate);
							continue;
						}

						/*
						 * allow other lock holders, too, but only if they have
						 * KeyShare
						 */
...
l4:
		...
		if (!(old_infomask & HEAP_XMAX_INVALID))
		{
			...
			else
			{
				...
				res = test_lockmode_for_conflict(status, xmax, mode,
												 &needwait);
				if (needwait)
				{
					LockBuffer(buf, BUFFER_LOCK_UNLOCK);
				  ...
				  uint16 *flags)
{
	...
	*flags = 0;
	...
	else if (MultiXactIdPrecedes(multi, cutoff_multi))
	{
		 ...
		 * preserve it.
		 */
		Assert(!MultiXactIdIsRunning(multi, false));
	...
	nmembers = GetMultiXactIdMembers(multi, &members, allow_old, false);
	if (nmembers <= 0)
	{
