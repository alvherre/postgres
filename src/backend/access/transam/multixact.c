...
GetNewMultiXactId(int nmembers, MultiXactOffset *offset)
{
	...
	/* give a change for ExtendMultiXactOffset to increment it */
 ...
 *
 * If the newly allocated page is the first page on the segment, additionally
 * store an appropriate truncate Xid value in the first position; the passed
 * multi is incremented and returned.  Otherwise, the original value is
 * returned.
 *
