						 ...
						 Oid *pf_eq_oprs, Oid *pp_eq_oprs, Oid *ff_eq_oprs)
{
	...
	if (isNull)
		elog(ERROR, "null confkey for constraint %u", HeapTupleGetOid(tuple));
	arr = DatumGetArrayTypeP(adatum);	/* ensure not toasted */
	if (ARR_NDIM(arr) != 1 ||
	...
	if (pf_eq_oprs)
	{
		...
		if (isNull)
			elog(ERROR, "null conpfeqop for constraint %u", HeapTupleGetOid(tuple));
		arr = DatumGetArrayTypeP(adatum);	/* ensure not toasted */
	...
	if (pp_eq_oprs)
	{
		...
		if (isNull)
			elog(ERROR, "null conppeqop for constraint %u", HeapTupleGetOid(tuple));
		arr = DatumGetArrayTypeP(adatum);	/* ensure not toasted */
		if (ARR_NDIM(arr) != 1 ||
			ARR_DIMS(arr)[0] != numkeys ||
			ARR_HASNULL(arr) ||
	...
	if (ff_eq_oprs)
	{
		...
		if (isNull)
			elog(ERROR, "null conffeqop for constraint %u", HeapTupleGetOid(tuple));
		arr = DatumGetArrayTypeP(adatum);	/* ensure not toasted */
		if (ARR_NDIM(arr) != 1 ||
			ARR_DIMS(arr)[0] != numkeys ||
			ARR_HASNULL(arr) ||
