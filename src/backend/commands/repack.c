...
typedef struct ChangeContext
{
	...
	/*
	 * The tuple descriptor to deform decoded tuples with.  This must have all
	 * attmissingval from the source relation, and match the attribute layout
	 * of the target relation; see apply_concurrent_changes().
	 */
...
apply_concurrent_changes(BufFile *file, ChangeContext *chgcxt)
{
	 ...
	 * worker.  It must be read using the tuple descriptor of the source
	 ...
	 * from, read from the spill file, so we also use the source rel's
	 * tupdesc.  (XXX Some places aren't prepared for that tupdesc's attribute
	 * layout to differ from the target rel's tupdesc though.)
	 *
	 * ondisk_tuple is the tuple in the new relation for UPDATEs and DELETEs,
	 * as obtained by searching by replication identity.  It uses tupdesc and
	 * tuptable ops appropriate for the new relation.
	 */
...
/*
 * Initialize ChangeContext to propagate changes of src_relation into
 * tgt_relation, using the given index (on tgt_relation) as identity.
 */
static void
