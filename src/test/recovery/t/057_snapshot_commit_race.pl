...
my $snappiest = PostgreSQL::Test::Cluster->new('primary');
$snappiest->init(allows_streaming => 'logical');
$snappiest->start;

if (!$snappiest->check_extension('injection_points'))
{
	plan skip_all => 'Extension injection_points not installed';
}

$snappiest->safe_psql('postgres', q(CREATE EXTENSION injection_points));
$snappiest->safe_psql(
	'postgres',
...
sub wait_for_xact_lock_wait
{
	...
	$snappiest->poll_query_until('postgres',
...
my $s2 = $snappiest->background_psql('postgres');
my $s3 = $snappiest->background_psql('postgres');
my $s4 = $snappiest->background_psql('postgres');
my $walsender =
  $snappiest->background_psql('postgres', replication => 'database');
...
my $s4_xid = $snappiest->safe_psql('postgres',
...
is($snappiest->safe_psql('postgres', 'SELECT i, j FROM tab ORDER BY i'),
